...
object JSBenchmark extends BenchmarkSuite {

  case class BenchmarkResult(
    name: String,
    jsTime: Option[Double] = None,
    wasmTime: Option[Double] = None
  )
  ...
  def main(args: Array[String]): Unit = {
    println("[JSBenchmark] Module loaded and main() called")

    def runBenchmarksWithProgress(): Unit = {
      val resultsElement = dom.document.getElementById("benchmark-results")

      if (resultsElement != null) {
        println("[JSBenchmark] Found results element, starting benchmarks")
        resultsElement.innerHTML = "<div style='color: #0066cc;'>Starting JavaScript benchmarks...</div>"

        js.timers.setTimeout(100) {
          try {
            resultsElement.innerHTML = "<div style='color: #0066cc;'>Running performance tests...</div>"
            val results = runBenchmarks()

            println(s"[JSBenchmark] Benchmarks complete: ${JSON.stringify(results)}")
            ...
            dom.window.asInstanceOf[js.Dynamic].jsBenchmarkResults = results
            ...
            dom.document.dispatchEvent(
              js.Dynamic.newInstance(js.Dynamic.global.CustomEvent)("js-benchmark-complete", event).asInstanceOf[dom.Event]
            )
          } catch {
            case e: Exception =>
              println(s"[JSBenchmark] Error: ${e.getMessage}")
              resultsElement.innerHTML = s"<div style='color: red;'>Error: ${e.getMessage}</div>"
          }
        }
      } else {
        println("[JSBenchmark] Results element not found, waiting...")
      }
    }
